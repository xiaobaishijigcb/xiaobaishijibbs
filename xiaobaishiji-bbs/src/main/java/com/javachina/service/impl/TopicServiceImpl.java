...
public class TopicServiceImpl implements TopicService {
	...
	@Override
	public Long save(Long uid, Long nid, String title, String content, Integer isTop,String videoPath,String picPath) {
		
		try {
			Integer time = DateKit.getCurrentUnixTime();
			
			Long tid = (Long) AR
					.update("insert into t_topic(uid, nid, title, content, is_top, create_time, update_time, status,video_path,pic_path) values(?, ?, ?, ?, ?, ?, ?, ?,?,?)",
							uid, nid, title, content, isTop, time, time, 1,videoPath,picPath).key();
	...
	public Map<String, Object> getTopicMap(Topic topic, boolean isDetail) {
		...
		map.put("video_path", topic.getVideo_path());
		map.put("pic_path", topic.getPic_path());
